...
```
Returns a list of [book request](#book-request-structure) objects.
...
POST /users/@me/books
```
Returns a [book sharing](#book-sharing-structure) object.
...
```
Returns a list of [book](#book-structure) objects.

## Request Book from User
```http
POST /users/{user.id}/books/{book.id}/request
```
Returns a [book request](#book-request-structure) object.
